...
DEPLOYED_BIN_PATH="${NARSIL_MCP_DEPLOYED_BIN:-}"
...
cleanup_temp_files() {
  if [[ -n "$DEPLOYED_BIN_TMP" ]]; then
    rm -f "$DEPLOYED_BIN_TMP"
  fi
  if [[ -n "$PLIST_TMP" ]]; then
    rm -f "$PLIST_TMP"
  fi
...
Options:
  ...
  --deployed-bin <path>   immutable daemon binary destination (default: <index-path>/bin/narsil-mcp)
...
if [[ -z "$DEPLOYED_BIN_PATH" ]]; then
  DEPLOYED_BIN_PATH="$INDEX_PATH/bin/narsil-mcp"
fi

mkdir -p "$(dirname "$DEPLOYED_BIN_PATH")"
if [[ "$BIN_PATH" != "$DEPLOYED_BIN_PATH" ]]; then
  DEPLOYED_BIN_TMP="$(mktemp "$(dirname "$DEPLOYED_BIN_PATH")/.narsil-mcp.XXXXXX")"
  cp "$BIN_PATH" "$DEPLOYED_BIN_TMP"
  chmod 0755 "$DEPLOYED_BIN_TMP"
  mv "$DEPLOYED_BIN_TMP" "$DEPLOYED_BIN_PATH"
  DEPLOYED_BIN_TMP=""
fi
BIN_PATH="$DEPLOYED_BIN_PATH"
...
echo "Staged daemon binary: $BIN_PATH"
echo "Label: $LABEL"
